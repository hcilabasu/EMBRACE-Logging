...
def createLabels():
    ...
    for bookKey, numQuestionsPerChapter in numQuestionsPerChapterByBook.items():
        # Generate label
        label = bookKey + " Order"
        ...
        for chapterNum in range(1, len(numQuestionsPerChapter) + 1):
            # Generate label
            ...
            numQuestions = numQuestionsPerChapter[chapterNum - 1]
            
            for questionNum in range(1, numQuestions + 1):
                # Generate label
                label = "{0} CH{1} Q{2}".format(bookKey, chapterNum, questionNum)
                
                # Write label
                worksheet.write(0, labelCol, label, bold)
                
                # Record column number
                chapterQuestionCol[label] = labelCol
...
if __name__ == "__main__":
    # Create Excel file
    workbook = xlsxwriter.Workbook("assessment_data.xlsx")
                    ...
                    if action == "Load Book":
                        ...
                        if bookCompletion[bookKey] == False:
                            ...
                            if bookTitle != prevBookTitle:
                                ...
                                numChapters = len(numQuestionsPerChapterByBook[bookKey])
                    ...
                    # Ignore any extra log data if book was already completed
                        ...
                        # Sentence audio indicates that the system was reading to the user
                            ...
                            if verification == "Correct":
                                ...
                                questionNumKey = "Q" + questionNum
                                
                                # Create key
                                key = "{0} {1} {2}".format(bookKey, chapterKey, questionNumKey)
                                
                                # Write number of attempts in appropriate column
                                col = chapterQuestionCol[key]
